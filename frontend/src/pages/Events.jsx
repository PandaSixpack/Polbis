...
const Events = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        payload.image = currentEvent.image;
      }

      if (currentEvent) {
        await api.put(`/events/admin/${currentEvent._id}`, payload);
        toast.success('Event updated successfully');
      } else {
        if (!selectedFile) {
          toast.error('Image is required for new event');
          setSubmitting(false);
          return;
        }
        await api.post('/events/admin', payload);
  ...
  const handleDelete = async () => {
    setSubmitting(true);
    try {
      await api.delete(`/events/admin/${currentEvent._id}`);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Organize and manage university events.</p>
        </div>
            ...
            <div key={item._id} className="card group hover:border-primary-500/50 transition-all flex flex-col">
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => { setCurrentEvent(item); setIsDeleteModalOpen(true); }} className="p-2 rounded-lg bg-white/90 dark:bg-slate-900/90 text-red-600 hover:bg-red-50 shadow-lg backdrop-blur-sm">
                    <Trash2 size={16} />
                  </button>
                </div>
                <div className="absolute bottom-3 left-3 px-3 py-1 rounded-full bg-slate-900/70 text-white text-xs font-bold backdrop-blur-sm flex items-center gap-1">
                  <Tag size={12} />
                  {item.category}
                </div>
              </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={currentEvent ? 'Edit Event' : 'Create Event'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <FileUpload label="Event Banner" onFileSelect={handleFileSelect} preview={imagePreview} onClear={() => { setSelectedFile(null); setImagePreview(''); }} />
          
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Title</label>
            <input {...register('title')} className={`input-field ${errors.title ? 'border-red-500' : ''}`} placeholder="Event title" />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select {...register('category')} className={`input-field ${errors.category ? 'border-red-500' : ''}`}>
          ...
          <div>
            ...
            <textarea {...register('description')} rows="4" className={`input-field ${errors.description ? 'border-red-500' : ''}`} placeholder="Write about the event..." />
